...
async def on_message(scp):
    ...
    if scp.content.startswith('!환영'): 
        ...
        try:
            ...
            embed=discord.Embed(title="환영합니다," + <@{str(info.id)}> + "님!", description="SCP 세계관 공식 한국어 사이트 대화방에 오신걸 환영합니다!", color=0x00ff56)
            ...
            embed=discord.Embed(title= f"뒤에 이름을 적어주세요.", description=f"예를 들어, !환영 Cresendo 이렇게 말이죠.", color=0xf3bb76)
            await scp.channel.send(embed=embed)
